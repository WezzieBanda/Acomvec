...
g2_dat <- read_csv("Wezzie-Tom/EHT DATA/Benin Trial/benin trial data results/stan/ellie code/pyrethroid-pyrrole nets April 2024 no unpublished data shown.csv")

library(dplyr)
g2_dat <- g2_dat %>%
  rename(
    n_dead_72h = N_72h_mort_B,
    n_total = N_total_B,
    n_dead_72h_PYRNET = N_dead_comb_pyrethroid,
    n_total_PYRNET = N_total_pyrethroid
  )
...
pbo_dat <- read_csv("Wezzie-Tom/EHT DATA/Benin Trial/benin trial data results/stan/ellie code/pyrethroid-pbo nets April 2024 no unpublished data shown.csv")
...
pbo_dat <- pbo_dat %>%
  rename(
    n_dead_24h = N_dead_comb_B,
    n_total = N_total_B,
    n_dead_24h_PYRNET = N_dead_comb_pyrethroid,
    n_total_PYRNET = N_total_pyrethroid
  )
...
#Extract data for Stan
data_list_g2_beta = list(N = nrow(data_PBO),
                         n = data_PBO$n_t,
                         d = data_PBO$d_t,
                         x = data_PBO$x1,
...
betbin_fit <- readRDS("C:/Users/user/Downloads/ento_pbo_beta_binomial_benefit_inv_link_LancetGH2024_extracted_samples (1).RDS")
...
g2_dat <- full_join(ig2_summary, ig1_summary, by = "Time")
...
# -----------------------------
axis(1, at = seq(0, 1, 0.2), labels = seq(0, 100, 20), cex.axis = 1.2)
axis(2, las = 2, at = seq(0, 1, 0.2), labels = seq(0, 100, 20), "~/Wezzie-Tom/EHT DATA/Benin Trial/benin trial data results/New folder/New folder/PBO Time.png"cex.axis = 1.2)
