...
# --- Time points (x-axis) ---
time <- c(1, 2, 3)   # e.g. 0, 10, 20 washes
n_smooth <- 300
x_smooth <- seq(min(time), max(time), length.out = n_smooth)

# --- Example probabilities ---
Killed    <- c(0.52, 0.45, 0.11)
Deterred  <- c(0.35, 0.28, 0.35)
Repelled  <- c(0.008, 0.10, 0.03)
Fed       <- 1 - (Killed + Deterred + Repelled)

# --- Smooth interpolation using spline() (works with 3 points) ---
...
my.cols <- c(
  adjustcolor("#E15759", alpha.f = 0.8),  # Fed (red)
  adjustcolor("#FAD02E", alpha.f = 0.6),  # Repelled (yellow)
  adjustcolor("#7AD151", alpha.f = 0.6),  # Deterred (green)
  adjustcolor("#6BAED6", alpha.f = 0.5)   # Killed (blue)
)

# --- Plot setup ---
par(bg = "white", mar = c(5,5,3,2), las=1)
plot(x_smooth, x_smooth, type="n", ylim=c(0,1), xlim=range(time),
     xlab="Time (e.g., washes or months)", ylab="Probability (%)",
     main="Pyrethroid–Pyrrole Net",
     cex.lab=1.3, cex.main=1.4, cex.axis=1.1,
     frame.plot = FALSE)

# --- Draw smooth stacked polygons ---
...
# --- Labels ---
text(2.5, 0.93, "Killed", col="#045A8D", cex=1.1)
text(2.0, 0.60, "Deterred", col="#238B45", cex=1.1)
text(2.0, 0.35, "Repelled", col="#B37E00", cex=1.1)
text(2.6, 0.13, "Fed", col="#A80000", cex=1.1)
